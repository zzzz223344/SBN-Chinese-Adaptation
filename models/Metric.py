...
class Metric():
    ...
    # span 是 [batch, start, end]
        start, end = int(span[1]), int(span[2])
        bert_span_index = [i for i, x in enumerate(bert_spans) if x[0] == start and x[1] == end]
        if len(bert_span_index) == 0:
            # 尝试再匹配一次（防止类型差异）
            bert_span_index = [i for i, x in enumerate(bert_spans) if int(x[0]) == start and int(x[1]) == end]
        if len(bert_span_index) == 0:
            print(f"Warning: cannot find aspect span ({start}, {end}) in bert_spans for sentence {sentence_index}")
            return 'none', 0.0
        idx = bert_span_index[0]
        sentiment_idx = aspect_sentiment[sentence_index][idx]
        sentiment = id4validity.get(sentiment_idx, 'none')
        logits = aspect_sentiment_logit[sentence_index][idx]
        sentiment_logit = logits[sentiment_idx] if sentiment_idx < len(logits) else 0.0
    ...
    def find_opinion_sentiment(self, sentence_index, opinion_index, bert_spans, span, opinion_sentiment,
        ...
        if len(bert_span_index) == 0:
            bert_span_index = [i for i, x in enumerate(bert_spans) if int(x[0]) == start and int(x[1]) == end]
        if len(bert_span_index) == 0:
            print(f"Warning: cannot find opinion span ({start}, {end}) in bert_spans for sentence {sentence_index}")
            return 'none', 0.0
        idx = bert_span_index[0]
        sentiment_idx = opinion_sentiment[sentence_index][opinion_index][idx]
        sentiment = id4sentiment.get(sentiment_idx, 'none')
        logits = opinion_sentiment_logit[sentence_index][opinion_index][idx]
        sentiment_logit = logits[sentiment_idx] if sentiment_idx < len(logits) else 0.0
        return sentiment, sentiment_logit
