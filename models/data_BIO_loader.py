...
def convert_examples_to_features(args, train_instances, max_span_length=8):
    ...
    # 如果情感不一致，取第一个（保留所有 opinion，但只用第一个情感）
            if len(set(sentiment_opinion)) != 1:
        # 可选：打印警告
        # print(f"警告：aspect {key_aspect} 的不同 opinion 情感不一致，取第一个")
                sentiment_opinion = [sentiment_opinion[0]]
        ...
        for opinion_key in aspect_reverse:
            ...
            if len(set(sentiment_aspect)) != 1:
        # 取第一个情感
                sentiment_aspect = [sentiment_aspect[0]]
            aspect_label2triple = []
            for i in spans:
...
def load_data_instances_txt(lines):
    sentiment2sentiment = {'NEG': 'negative', 'POS': 'positive', 'NEU': 'neutral' , 'Neutral': 'neutral',   'Negative': 'negative','Positive': 'positive'}
    instances = list()
    triples_num = 0
    aspects_num = 0
