...
def stage_2_features_generation(bert_feature, attention_mask, spans, span_mask, spans_embedding, spans_aspect_tensor,
    ...
    # 确保 spans_embedding 是 3 维 (batch, num_spans, hidden_size)
    hidden_size = spans_embedding.size(-1)
    spans_embedding = spans_embedding.view(spans_embedding.size(0), -1, hidden_size)
...
# ---- 新增：只取第一个匹配的 span ----
        if span_index.numel() > 1:
            span_index = span_index[0].unsqueeze(0)
# ----------------------------------------
        ...
        if spans_opinion_tensor is not None:
            ...
            # 压缩多余维度
            if spans_opinion_tensor_unspilt.dim() == 4:
                spans_opinion_tensor_unspilt = spans_opinion_tensor_unspilt.squeeze(1)

        # 提取 aspect span embedding
        aspect_span_embedding_unspilt = spans_embedding[batch_num, span_index, :].unsqueeze(0)
        if aspect_span_embedding_unspilt.dim() == 4:
            aspect_span_embedding_unspilt = aspect_span_embedding_unspilt.squeeze(1)
        ...
        if all_span_aspect_tensor is None:
            ...
            # ----- 强制统一维度后再拼接 -----
            # 1. 处理 all_span_opinion_tensor
            if spans_opinion_tensor is not None:
                if all_span_opinion_tensor.dim() == 4:
                    all_span_opinion_tensor = all_span_opinion_tensor.squeeze(1)
                if spans_opinion_tensor_unspilt.dim() == 4:
                    spans_opinion_tensor_unspilt = spans_opinion_tensor_unspilt.squeeze(1)
                # 确保两者都是 3 维（batch, num_aspect, max_spans）
                if all_span_opinion_tensor.dim() != 3:
                    all_span_opinion_tensor = all_span_opinion_tensor.view(all_span_opinion_tensor.size(0), -1,
                                                                            all_span_opinion_tensor.size(-1))
                if spans_opinion_tensor_unspilt.dim() != 3:
                    spans_opinion_tensor_unspilt = spans_opinion_tensor_unspilt.view(
                        spans_opinion_tensor_unspilt.size(0), -1, spans_opinion_tensor_unspilt.size(-1))
                all_span_opinion_tensor = torch.cat((all_span_opinion_tensor, spans_opinion_tensor_unspilt), dim=0)

            # 2. 处理 all_span_aspect_tensor 和 aspect_span_embedding_unspilt
            if all_span_aspect_tensor.dim() == 4:
                all_span_aspect_tensor = all_span_aspect_tensor.squeeze(1)
            if aspect_span_embedding_unspilt.dim() == 4:
                aspect_span_embedding_unspilt = aspect_span_embedding_unspilt.squeeze(1)
            if all_span_aspect_tensor.dim() != 3:
                all_span_aspect_tensor = all_span_aspect_tensor.view(all_span_aspect_tensor.size(0), -1, hidden_size)
            if aspect_span_embedding_unspilt.dim() != 3:
                aspect_span_embedding_unspilt = aspect_span_embedding_unspilt.view(
                    aspect_span_embedding_unspilt.size(0), -1, hidden_size)
...
class Step_1(torch.nn.Module):
    ...
    def span_generator(self, input_bert_features, attention_mask, spans, span_mask, related_spans_tensor,
        ...
        if self.args.span_generation == "Average" or self.args.span_generation == "Max":
            ...
            spans_embedding = span_sum_embdding.squeeze(2)
        elif self.args.span_generation == "SE_ATT":
            spans_width_start_end = spans[:, :, 0:2].view(spans.shape[0], spans.shape[1], -1)
            spans_width_start_end_embedding, spans_width_start_end_mask = batched_span_select(bert_feature,
                                                                                              spans_width_start_end)
